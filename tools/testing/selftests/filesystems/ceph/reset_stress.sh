...
discover_client_id()
{
	...
	if [[ -n "$CLIENT_ID" ]]; then
		if ! $SUDO test -d "$DEBUGFS_ROOT/$CLIENT_ID/reset"; then
			echo "reset debugfs not found for client-id=$CLIENT_ID" >&2
			exit 2
		fi
		return 0
	fi
	...
	if ! $SUDO test -d "$DEBUGFS_ROOT"; then
		echo "Debugfs root not found: $DEBUGFS_ROOT" >&2
		exit 2
	fi
	...
	echo "Multiple Ceph clients found (${candidates[*]}). Use --client-id." >&2
	exit 2
}
...
main()
{
	...
	if [[ "$EXPECT_RESET" -eq 1 ]]; then
		...
		if ! $SUDO test -w "$trigger_path"; then
			echo "Reset trigger is not writable: $trigger_path" >&2
			exit 2
		fi
		if ! $SUDO test -r "$status_path"; then
			echo "Reset status is not readable: $status_path" >&2
			exit 2
		fi
