...
int ceph_mdsc_wait_for_reset(struct ceph_mds_client *mdsc)
{
	 ...
	 * reset failed, sessions may be in an inconsistent state, so
	 * failing requests forces the operator to investigate rather
	 * than silently proceeding with potentially stale state.
	 *
	 * Note: a narrow TOCTOU window exists between the wake-up and
	 * the spinlock acquisition below — if another CPU calls
	 * schedule_reset() in that window, last_errno may read as 0
	 * instead of the prior failure.  This is benign: the failing
	 * reset is already being retried and the request will proceed
	 * into a fresh reset cycle.
	 */
	spin_lock(&st->lock);
	ret = st->last_errno;
	spin_unlock(&st->lock);
...
static void mds_peer_reset(struct ceph_connection *con)
{
	 ...
	 *
	 * Snapshot session state, then use the drop-and-reacquire
	 * pattern: take mdsc->mutex for unregistration (checking the
	 * session is still registered and state unchanged), then take
	 * s->s_mutex separately for cleanup.  This avoids introducing
	 * a new simultaneous s->s_mutex + mdsc->mutex nesting.
	 */
	mutex_lock(&s->s_mutex);
