 ...
 *
 * cf->ci is safe to dereference here: the inode remains alive while its
 * ceph_cap_flush is on i_cap_flush_list, which holds a reference.
 */
...
static void mds_peer_reset(struct ceph_connection *con)
{
	 ...
	 * (consistent with check_new_map() and cleanup_session_requests(),
	 * which achieve the same ordering via drop-and-reacquire).
	 *
	 * An ABA race (e.g. OPEN -> CLOSED -> OPEN) between the snapshot
	 * and re-acquire is prevented by mdsc->mutex serialization of
	...
	case CEPH_MDS_SESSION_RECONNECTING: {
		...
		pr_info_client(mdsc->fsc->client,
			       ...
			       s->s_mds);
		/*
		 * Preserve from_reset if this reconnect was initiated
		 * by a manual reset, so lock reclaim remains unconditional.
		 */
		was_reset = (s->s_reset_gen != 0);
		rc = send_mds_reconnect(mdsc, s, was_reset);
		if (rc) {
			pr_err_client(mdsc->fsc->client,
